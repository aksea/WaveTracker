...
基于原有的gui_camera_test.py，增加进程间通信功能
支持主进程的志愿者姓名同步和录制控制
"""
...
class CameraSubprocessGUI(QMainWindow):
    """摄像头子进程GUI主窗口"""
    ...
    def on_volunteer_name_received(self, volunteer_name: str):
        ...
        self.on_volunteer_name_changed()
    ...
    def init_ui(self):
        """初始化UI"""
        self.setWindowTitle(f"摄像头功能测试 (子进程 - {self.instance_id})")
    ...
    def create_display_area(self) -> QWidget:
        ...
        self.recording_status_label = QLabel("● 等待主进程控制")
    ...
    def create_control_panel(self) -> QWidget:
        ...
        process_info_label = QLabel(f"此窗口为摄像头子进程 ({self.instance_id})")
        ...
        # 数据路径显示
        self.data_path_label = QLabel("等待主进程同步志愿者姓名")
        ...
        # 默认分辨率显示（从配置文件读取，只读）
        default_width = self.config_manager.get("camera.connection.default_resolution.width", 640)
        default_height = self.config_manager.get("camera.connection.default_resolution.height", 480)
        ...
        # 录制组（显示状态，由主进程控制）
        record_group = QGroupBox("视频录制 (主进程控制)")
        record_layout = QVBoxLayout()
        record_group.setLayout(record_layout)
        
        record_status_info = QLabel("录制控制由主进程统一管理")
        record_status_info.setStyleSheet("color: gray; font-style: italic;")
        record_layout.addWidget(record_status_info)
    ...
    def on_volunteer_name_changed(self):
        ...
        if self.volunteer_name:
            ...
            self.data_path_label.setText("等待主进程同步志愿者姓名")
            self.data_path_label.setStyleSheet("color: gray; background-color: #f0f0f0; padding: 5px; border: 1px solid #ccc;")
    ...
    def start_recording_all(self):
        ...
        for cam_id in self.connected_devices.keys():
            if self.camera_recorder and self.camera_recorder.is_recording(cam_id):
                ...
                continue
            device_info = self.connected_devices[cam_id]
            display_name = device_info['display_name']
            output_path = data_path / f"{display_name}_{timestamp}.mp4"

            self.log_message(f"开始录制摄像头 {cam_id} ({display_name}) 到 {output_path}")
    ...
    def stop_recording_all(self):
        ...
        # 录制状态将通过 on_recording_stopped 信号更新
        self.update_recording_status()
    ...
    def update_recording_status(self):
        ...
        if is_any_recording:
            ...
            self.recording_status_label.setText("● 等待主进程控制")
            self.recording_status_label.setStyleSheet("color: gray; font-weight: bold;")

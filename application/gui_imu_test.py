...
使用PyQt5和pyqtgraph显示IMU的6DOF数据（加速度和角速度）
支持多设备连接、选择显示和数据录制
通过 IMURecorder 实现生产者-消费者架构
"""
...
class IMUPlotWidget(QWidget):
    ...
    def __init__(self, parent_gui: 'IMUTestGUI'):
        """
        初始化绘图组件
        Args:
...
class IMUTestGUI(QMainWindow):
    ...
    def init_ui(self):
        """初始化UI"""
        self.setWindowTitle("IMU 多设备6DOF数据实时显示和录制")
    ...
    def create_control_panel(self):
        ...
        # 录制管理组
        record_group = QGroupBox("录制管理")
        record_layout = QVBoxLayout()
        record_group.setLayout(record_layout)
        
        # 录制按钮
        record_button_layout = QHBoxLayout()
        self.start_record_button = QPushButton("开始录制选中设备")
        self.start_record_button.clicked.connect(self.start_recording_selected)
        self.start_record_button.setEnabled(False)
        record_button_layout.addWidget(self.start_record_button)
        
        self.start_record_all_button = QPushButton("开始录制所有设备")
        self.start_record_all_button.clicked.connect(self.start_recording_all)
        self.start_record_all_button.setEnabled(False)
        record_button_layout.addWidget(self.start_record_all_button)
        
        record_layout.addLayout(record_button_layout)
        
        stop_record_layout = QHBoxLayout()
        self.stop_record_button = QPushButton("停止录制选中设备")
        self.stop_record_button.clicked.connect(self.stop_recording_selected)
        self.stop_record_button.setEnabled(False)
        stop_record_layout.addWidget(self.stop_record_button)
        
        self.stop_record_all_button = QPushButton("停止录制所有设备")
        self.stop_record_all_button.clicked.connect(self.stop_recording_all)
        self.stop_record_all_button.setEnabled(False)
        stop_record_layout.addWidget(self.stop_record_all_button)
        
        record_layout.addLayout(stop_record_layout)
        ...
        self.record_status_label = QLabel("录制状态: 无")
    ...
    def start_recording_selected(self):
        """开始录制选中设备"""
        current_item = self.connected_list.currentItem()
        if current_item and self.imu_recorder:
            address = current_item.data(Qt.ItemDataRole.UserRole)
    ...
    def _start_recording_device(self, address: str):
        ...
        device_name = self.connected_devices[address].display_name.replace(' ', '_')
    ...
    def stop_recording_selected(self):
        """停止录制选中设备"""
        current_item = self.connected_list.currentItem()
        if current_item and self.imu_recorder:
            address = current_item.data(Qt.ItemDataRole.UserRole)
    ...
    def on_device_connected(self, address):
        ...
        if address in self.discovered_devices:
            ...
            self.start_record_all_button.setEnabled(True)
    ...
    def on_recording_started(self, address):
        ...
        if address in self.connected_devices:
            ...
            self.stop_record_all_button.setEnabled(True)
            self.update_record_status_display()
